...
A `Controller` (`src/application/controllers/controller.ts`) takes a use case in its constructor and exposes `handle(request: ControllerRequest): Promise<ControllerResponse>` — plain objects, no dependency on `aws-lambda` or any other trigger-specific library. It owns request validation and mapping domain errors to status codes.
