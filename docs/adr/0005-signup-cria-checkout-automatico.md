...
> **Substituída pela [ADR-0009](./0009-signup-checkout-e-opt-in-via-creditsqty.md).** `SignUpUser` não cria mais um checkout com quantidade fixa por padrão — `creditsQty` passou a ser um parâmetro opcional informado pelo caller, e sem ele nenhum checkout é criado.
