...
- [Extrair useOnClickOutside pra hooks/app/](issues/09-extrair-useonclickoutside.md) — `hooks/app/useOnClickOutside.ts` criado (ref único ou array, pra suportar o caso de `Select.tsx` com trigger + listbox portado); `UserMenu`, `Select` e `HighlightedEssayText` migrados preservando comportamento exato, incluindo o scroll/resize extra de `HighlightedEssayText` mantido local
...
- [Extrair tipos de entidade dos services para types/](issues/05-extrair-types-de-services.md) — criado `frontend/src/types/{essay,theme,topic,user,auth}.ts` com as formas de entidade extraídas de `services/*-service.ts` (kebab-case, ticket 04 ainda não rodou), services reimportando os tipos que ainda usam e 11 consumidores repontados pra `types/*`
- [CamelCase em arquivos e diretórios de página/service/lib/schema](issues/04-camelcase-em-arquivos-e-diretorios.md) — `pages/essay-result/`→`pages/essayResult/`, `theme-detail.tsx`→`themeDetail.tsx`, `login-schema.ts`/`signup-schema.ts`→`loginSchema.ts`/`signupSchema.ts`, os 6 `services/*-service.ts`→`*Service.ts` e `libs/form-errors.ts`/`libs/hard-shadow.ts`→`formErrors.ts`/`hardShadow.ts`, com todos os importers em `frontend/src` repontados e `tsc -b` limpo
- [Extrair rotas de App.tsx pra src/routes.tsx](issues/10-extrair-rotas-de-app.md) — `RootRoute`/`RequireAuth`/árvore de `<Routes>` movidos pra `routes.tsx` (default export `AppRoutes`), `App.tsx` reduzido a `AuthProvider`+`Toaster`+`AppRoutes`, `tsc -b` limpo
