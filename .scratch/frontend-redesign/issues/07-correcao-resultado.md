...
- Novo componente de design system `PaperCard` (papel pautado com borda recortada) — 2 usos nessa ticket (texto da redação + nota final), asset `assets/paper-edge.svg` baixado do Figma (borda recortada), linhas do papel são CSS (`repeating` via `background-image`/`background-size`), não asset, pra suportar texto de tamanho variável. Token novo `--color-neutral-50: #c6c8c7` (cor da linha). `Bullet` ganhou variante `dark-outline` (mesmo visual de `dark`, borda preta em vez de branca — pro uso direto sobre fundo claro).
