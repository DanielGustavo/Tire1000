...
- [Tom mais direto nos textos de parecer e destaque](issues/03-tom-mais-direto-nos-pareceres-e-destaques.md) — instruções de `evaluationText` e `highlights[].textContent` em `domain/ai/evaluation/prompt.ts` reescritas pra tom direto/coloquial (sem saudação/despedida nos 5 pareceres de competência, exclusivo do parecer geral da ticket 02); limite de `highlights[].textContent` relaxado pra até 3-4 frases quando ajudar a ser explicativo. `POPUP_WIDTH_PX` em `HighlightedEssayText.tsx` de `260` pra `320`; cálculo de `left` deriva da constante, sem mudança de lógica. Sem teste dedicado (mudança de texto livre + constante).
